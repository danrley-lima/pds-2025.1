...
def buscar_produtos(itens: List[ProdutoOut]) -> List[ProdutoOut]:
    ...
    resultado = []
    for prod in itens:
        produto = produtos_dict.get(str(prod.id))
        if not produto:
            continue
        # qtd_receita = parse_quantidade(prod.quantidade)
        ...
        resultado.append(
            ProdutoOut(
                ...
                quantidade_total=str(produto.get("stockQuantity", "")),
