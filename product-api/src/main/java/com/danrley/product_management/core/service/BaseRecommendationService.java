...
import com.danrley.product_management.common.exception.custom.RecommendationException;
...
public abstract class BaseRecommendationService {
  ...
  protected ProductOutDTO createFromProduct(BaseProduct product, String quantity) {
    ...
    dto.setCategoryName(product.getCategory() != null ? product.getCategory().getName() : getDefaultCategoryName());
    dto.setStockQuantity(String.valueOf(product.getStockQuantity())); 
  ...
  protected RecommendationResponseDTO createErrorResponse(String aiResponse) {
    ...
    fallback.setStockQuantity(aiResponse.length() > 500 ? aiResponse.substring(0, 500) + "..." : aiResponse);
